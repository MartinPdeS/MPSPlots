...
class ClassRepr(object):
    ...
    def __repr__(self):
        self.ReprEval = [f"{item:12s}" + f": \t{getattr(self, item)}" for item in self.ReprEval]

        self.Methods = [e + f"{signature(getattr(self, e))}" for e in self.Methods]
